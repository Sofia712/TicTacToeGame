...
def computerTurn(board, depth):
    ...
    for i in range(3):
        for j in range(3):
            if board[i][j] == '   ':
                board[i][j] = ' x '
                score = minimax(board, i, j,  depth, False)
                if score > maxScore:
                    move_r = i
                    move_c = j
                    maxScore = score
...
def minimax(board, move_i, move_j,  depth, isMax):
    #computer turn
    ...
    if isMax:
        ...
        for row in range(3):
            for col in range(3):
                if board[row][col] == '   ':
                    board[row][col] = ' x '
                    sc = minimax(board, row, col, depth + 1, False)
        ...
        for row in range(3):
            for col in range(3):
                if board[row][col] == '   ':
                    board[row][col] = ' o '
                    sc = minimax(board, row, col, depth + 1, True)
                    if sc < minScore:
                        minScore = sc
